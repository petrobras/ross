...
- **Scripts and notebooks** (``.py`` / ``.ipynb``): the keyword arguments of the constructors
  ...
  ``Probe(node, angle, tag=tag)`` objects (a ``probe_units="deg"`` keyword is folded into the angle
  as ``Q_(angle, "deg")`` and dropped), and anything that cannot be rewritten safely — positional
...
time and stochastic response plots had been deprecated since version 1.4.1 (2023); they were removed and
the ``probe_units`` keyword, which only applied to tuples, no longer has any effect. Build
``Probe`` objects instead, with a pint quantity when the angle is not in radians
(``Probe(3, Q_(45, "deg"), tag="DE")``); ``ross_2to3`` rewrites the call sites.
