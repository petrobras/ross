...
class MotorElement(Element):
    ...
    def _calculate_dLds_dt(self, Lds, Lqs, Ldr, vds, w_shaft):
    ...
    def _calculate_dLqs_dt(self, Lds, Lqs, Lqr, vqs, w_shaft):
    ...
    def _calculate_dLdr_dt(self, Lds, Ldr, Lqr, wr, vdr, w_shaft):
    ...
    def _calculate_dLqr_dt(self, Lqs, Ldr, Lqr, wr, vqr, w_shaft):
    ...
    def _calculate_dwr_dt(self, wr, Tl, Te):
    ...
    def _calculate_dflux_angle_dt(self, w_shaft):
        """Calculate time derivative of electromagnetic flux angle.

        Parameters
        ----------
        w_shaft : float
            Shaft angular speeds [rad/s].

        Returns
        -------
        float
            Time derivatives of electromagnetic flux angle [rad/s].
        """
        return w_shaft
    ...
    def _ode_system(
        self, t, y, load_torque_entrance_time, load_torque_ratio, element
    ):
        ...
        y : array_like
            ...
            - flux_angle : Electromagnetic flux angle [rad]
        ...
        Lds, Lqs, Ldr, Lqr, wr, flux_angle = y

        Tl = self._calculate_load_torque(
            t, load_torque_entrance_time, load_torque_ratio
        )

        Te = self._calculate_electrical_torque(Lds, Lqs, Ldr, Lqr)
        ...
        dLds_dt = self._calculate_dLds_dt(Lds, Lqs, Ldr, vds, w_shaft)
        dLqs_dt = self._calculate_dLqs_dt(Lds, Lqs, Lqr, vqs, w_shaft)
        dLdr_dt = self._calculate_dLdr_dt(Lds, Ldr, Lqr, wr, vdr, w_shaft)
        dLqr_dt = self._calculate_dLqr_dt(Lqs, Ldr, Lqr, wr, vqr, w_shaft)
        dwr_dt = self._calculate_dwr_dt(wr, Tl, Te)
        dflux_angle_dt = self._calculate_dflux_angle_dt(w_shaft)

        return dLds_dt, dLqs_dt, dLdr_dt, dLqr_dt, dwr_dt, dflux_angle_dt
    ...
    def _run(
        ...
        solution = sp.integrate.solve_ivp(
            ...
            t_span=(t[0], t[-1]),
            y0=(Lds0, Lqs0, Ldr0, Lqr0, wr0, flux_angle_0),
            args=(load_torque_entrance_time, load_torque_ratio, element),
            t_eval=t,
            # method="RK23",
        )

        Lds, Lqs, Ldr, Lqr, speed, flux_angle = solution.y
