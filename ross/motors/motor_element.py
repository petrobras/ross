...
class MotorElement(Element):
    ...
    def _ode_system(
        ...
        """State derivatives used by RK4."""

        Te = self._calculate_electrical_torque(Lds, Lqs, Ldr, Lqr)

        Rs = self.stator_resistance + self.short_circuit_resistance

        dLds_dt = vds - Rs * self.a * Lds + Rs * self.c * Ldr + w_shaft * Lqs
        dLqs_dt = vqs - Rs * self.a * Lqs + Rs * self.c * Lqr - w_shaft * Lds

        Rr = self.rotor_resistance
        w = w_shaft - wr * self.n_poles / 2

        dLdr_dt = vdr - Rr * self.b * Ldr + Rr * self.c * Lds + w * Lqr

        dLqr_dt = vqr - Rr * self.b * Lqr + Rr * self.c * Lqs - w * Ldr

        dwr_dt = (Te - self.viscosity_coeff * wr - Tl) / (self.Ip_motor + self.Ip_load)
    ...
    def _run(
        ...
        ids = self.a * Lds_hist - self.c * Ldr_hist
        iqs = self.a * Lqs_hist - self.c * Lqr_hist
