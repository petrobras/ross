    ...
    while (np.linalg.norm((T_novo-Tcomp) / np.linalg.norm(Tcomp)) > 0.01):
        ...
        for ii in range(N1+0.5*dN, dN, N2-0.5*dN):
            ...
            for jj in range(ksi1+0.5*dksi, dksi, ksi2-0.5*dksi):
                ...
                He=Rs-R-(np.sin[theta+0.5*dtheta]*(yr+alpha*(Rs+esp))+np.cos[theta+0.5*dtheta]*(xr+Rs-R-Cr))
                Hw=Rs-R-(np.sin[theta-0.5*dtheta]*(yr+alpha*(Rs+esp))+np.cos[theta-0.5*dtheta]*(xr+Rs-R-Cr))
                Hn=HP
                Hs=Hn
                Hnw=Hw
                Hsw=Hnw
                ...
                if (ki==1 and kj==1):
                    ...
                    b[k,0]=b[k,0]-2*(Aw*Tmist[ki+1]+Asw*Tmist[ki+2])-Anw*(Tmist[ki])
                ...
                if (kj==1 and ki>1 and ki<nN):
                    ...
                    b[k,0]=b[k,0]-2*Tmist[ki-1]*Anw-2*Tmist[ki+1]*Aw-2*Tmist[ki+2]*Asw
                ...
                if (kj==1 and ki==nN):
                    ...
                    Mat_coef[k,k-ntheta]=An-Anw
                    Mat_coef[k,k-ntheta+1]=Ane
                    b[k,0]=b[k,0]-2*Tmist[ki+1]*Aw-2*Tmist[ki]*Anw-Tmist[ki+2]*Asw
