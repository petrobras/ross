...
class Tilting:
    ...
    def __init__(
        ...
        TETA2 = (
            1 - self.rp_pad
        ) * self.betha_p  # final coordinate in the TETA diretion
        self.TETA1 = TETA1
        self.TETA2 = TETA2
        ...
        self.dimForca = 1 / (
            self.Cr**2 / (self.Rp**3 * self.mi0 * self.speed * self.L)
        )
    ...
    def run(self):
        ...
        self.TT_i = self.T0 * np.ones(
            (self.nZ, self.nX, self.npad)
        )  # Inicial 3D - temperature field
        ...
        if "calc_EQ" in self.op_key:
            ...
            self.WX = self.Wx * (
                self.Cr**2 / (self.Rp**3 * self.mi0 * self.speed * self.L)
            )  # Loading - X direction [dimensionless]
            self.WY = self.Wy * (
                self.Cr**2 / (self.Rp**3 * self.mi0 * self.speed * self.L)
    ...
    def PandTsolution(self):
        ...
        for n_p in range(0, self.npad):
                ...
                for ii in range(0, self.nZ):
                    ...
                    for jj in range(0, self.nX):
                        ...
                        hP = (
                            ...
                            - self.Rs
                            - (
                                np.sin(self.XTETA[jj])
                                * (yr + alpha * (self.Rp + self.tpad))
                                + np.cos(self.XTETA[jj])
                                * (xr + self.Rp - self.Rs - self.Cr)
                            )
                        ) / self.Cr
                        ...
                        he = (
                            ...
                            - self.Rs
                            - (
                                np.sin(TETAe) * (yr + alpha * (self.Rp + self.tpad))
                                + np.cos(TETAe) * (xr + self.Rp - self.Rs - self.Cr)
                            )
                        ) / self.Cr
                        ...
                        hw = (
                            ...
                            - self.Rs
                            - (
                                np.sin(TETAw) * (yr + alpha * (self.Rp + self.tpad))
                                + np.cos(TETAw) * (xr + self.Rp - self.Rs - self.Cr)
                            )
                        ) / self.Cr
                        ...
                        CP = -(CE + CW + CN + CS)
                        B = (self.Rs / (2 * self.Rp * self.betha_p)) * self.dZ * (
                            he - hw
                        ) + hpt * self.dX * self.dZ
                        b[k] = B
            ...
            self.TT_i[:, :, n_p] = T_new
            self.PPdim = (
                self.PP * (self.mi0 * self.speed * self.Rp**2) / (self.Cr**2)
            )
            self.h_pivot[n_p] = (
                self.Cr
                * (
                    ...
                    - (
                        ...
                        + np.cos(0) * (xr + self.Rp - self.Rs - self.Cr)
                    )
                )
                / self.Cr
            )
        ...
        ##### Outputs
        maxP = (
            self.PP[:, :, self.pad_in].max()
            * (self.mi0 * self.speed * self.Rp**2)
            / (self.Cr**2)
        )
        medP = (
            self.PP[:, :, self.pad_in].mean()
            * (self.mi0 * self.speed * self.Rp**2)
            / (self.Cr**2)
        )
    ...
    def coeffs_din(self):
        ...
        for a_p in range(0, 4):
            for n_p in range(0, self.npad):
                ...
                while abs((T_new - T_i).max()) >= tol_T:
                    ...
                    for ii in range(0, self.nZ):
                        for jj in range(0, self.nX):
                            ...
                            dudx = (
                                (HP / self.mi_turb[ii, jj, n_p]) * self.dPdX[ii, jj]
                            ) - (self.speed / HP)

                            dwdx = (HP / self.mi_turb[ii, jj, n_p]) * self.dPdZ[ii, jj]

                            tal = self.mi_turb[ii, jj, n_p] * np.sqrt(
                                (dudx**2) + (dwdx**2)
                            )
                            ...
                            AE = -(self.kt * HP * self.dZ) / (
                                ...
                                * self.Cp
                                * self.speed
                                * ((self.betha_p * self.Rp) ** 2)
                                * self.dX
                            )

                            AW = (
                                ((HP**3) * self.dPdX[ii, jj] * self.dZ)
                                / (12 * mi_t * (self.betha_p**2))
                                - (
                                    (self.Rs * HP * self.dZ)
                                    / (2 * self.Rp * self.betha_p)
                                )
                                - (self.kt * HP * self.dZ)
                                / (
                                    self.rho
                            ...
                            auxB_t = (self.speed * self.mi0) / (
                                self.rho * self.Cp * self.Tcub * self.Cr
                            )
                            ...
                            B_tG = (
                                ...
                                * self.P[ii, jj]
                                * hpt
                            ) / (self.rho * self.Cp * self.T0 * (self.Cr**2))
                            ...
                            B_tH = (
                                ...
                                * self.dX
                                * self.dZ
                            ) / (self.rho * self.Cp * self.T0 * 3 * HP)

                            B_tI = (
                                auxB_t
                                * (1 * mi_t * (self.Rs**2) * self.dX * self.dZ)
                                / (HP * self.Cr)
                            )
                            ...
                            B_tJ = (
                                ...
                                * (
                                ...
                                / (12 * self.Cr * (self.betha_p**2) * mi_t)
                            )
                            ...
                            if ii == 0 and jj == 0:
                                ...
                                test_diag[k] = Mat_coef_t[k, k] - (
                                    Mat_coef_t[k, k + 1] + Mat_coef_t[k, k + self.nX]
                                )
                ...
                # Hydrodynamic forces =================================================================
                P_dimen = (
                    self.P * (self.mi0 * self.speed * self.Rp**2) / (self.Cr**2)
                )
                ...
                # Angular speed perturbation
                    ...
                    self.Aj[n_p] = [
                        [self.Sjpt[n_p, 0, 0], self.Sjpt[n_p, 0, 1]],
                        [self.Sjpt[n_p, 1, 0], self.Sjpt[n_p, 1, 1]],
                    ]
                    self.Hj[n_p] = [[self.Sjpt[n_p, 0, 2]], [self.Sjpt[n_p, 1, 2]]]
                    self.Bj[n_p] = [[self.Sjpt[n_p, 2, 2]]]
                    self.Vj[n_p] = [[self.Sjpt[n_p, 2, 0], self.Sjpt[n_p, 2, 1]]]

                    self.Sj[n_p] = (
                        self.Aj[n_p]
                        - self.Hj[n_p] * np.linalg.inv(self.Bj[n_p]) * self.Vj[n_p]
                    )
                    ...
                    self.Sw = (
                        self.Sw
                        + ((np.transpose(self.Tj[n_p])) * self.Sj[n_p]) * self.Tj[n_p]
                    )
    ...
    def HDEequilibrium(self, x):
                ...
                for ii in range(0, self.nZ):
                    for jj in range(0, self.nX):
                        ...
                        AW = (
                            ((HP**3) * self.dPdX[ii, jj] * self.dZ)
                            / (12 * mi_t * (self.betha_p**2))
                            - ((self.Rs * HP * self.dZ) / (2 * self.Rp * self.betha_p))
                            - (self.kt * HP * self.dZ)
                            / (
                                self.rho
                                * self.Cp
                        ...
                        AN_1 = (aux_up - 1) * (
                            ((self.Rp**2) * (HP**3) * self.dPdZ[ii, jj] * self.dX)
                            / (12 * (self.L**2) * mi_t)
                        )
        ...
        if "calc_EQ" in self.op_key:
            for k_i in range(0, self.npad):
                self.Fx[k_i] = self.F1[k_i] * np.cos(
                    x[k_i + 2] + self.sigma[k_i]
                )  # angulo + posição angular (pivô)
    ...
    def alpha(self):
        alphamax = (
            self.Rp
            - self.Rs
            - np.cos(self.TETA2) * (self.xryr[0, 0] + self.Rp - self.Rs - self.Cr)
        ) / (sin(self.TETA2) * (self.Rp + self.tpad)) - (self.xryr[1, 0]) / (
            self.Rp + self.tpad
        )
        return abs(alphamax)
...
def tilting_pad_example02():
    ...
    bearing = Tilting(
        ...
        choice_CAIMP={
            "impos_EQ": {
                "pos_EQ": [0.300000000000001, 0.000001],
                "ent_angle": [
                    0.000636496725369308,
                ],
                "print": ["result", "progress"],
