...
from ross.interface.tests import repository
from ross.interface.domain.analysis_catalog import ANALYSES

# Where the folder lives inside the ROSS repository, and what it is called as
# a package. Written down because several guards depend on them.
FOLDER_IN_ROSS = "ross/interface"
PACKAGE = "ross.interface"
REPO = os.path.dirname(os.path.dirname(ROOT))


# --- the hook that keeps us out of a `pytest ross` that cannot import us ------


def test_the_hook_hides_us_where_the_interface_extra_is_missing(monkeypatch):
    """`pytest --pyargs ross` on an installation without Flask must still pass.

    ROSS's `pytest.ini` turns on `--doctest-modules`, which **imports** every
    module collection finds. Without the `interface` extra the import of our
    modules fails, and ROSS's suite would break because of the interface. So
    collection is conditional on the extra being importable."""
    from ross.interface import conftest

    assert conftest.the_interface_can_be_imported(), (
        "this suite is running: Flask has to be importable"
    )
    assert conftest.pytest_ignore_collect(ROOT, None) is False
    monkeypatch.setattr(conftest, "the_interface_can_be_imported", lambda: False)
    assert conftest.pytest_ignore_collect(ROOT, None) is True
...
def _pyproject():
    """ROSS's `pyproject.toml`, parsed -- or a skip when this is not a checkout.

    Installed from a wheel, `pytest --pyargs ross.interface` runs this suite from
    site-packages, where the repository's pyproject does not exist. The guards
    that read it are about the repository, so they have nothing to say there.
    """
    import tomllib

    path = os.path.join(REPO, "pyproject.toml")
    if not os.path.exists(path):
        pytest.skip("not a repository checkout: ROSS's pyproject.toml is absent")
...
def _ross_find_config():
    ...
    return _pyproject()["tool"]["setuptools"]["packages"]["find"]
...
def test_the_folder_ships_inside_the_ross_wheel_and_nothing_else_does(tmp_path):
    """ROSS's package discovery reaches this folder, and only ROSS's own folders.

    The interface is `ross.interface`: `pip install "ross-rotordynamics[interface]"`
    has to deliver it, so discovery must find it. `[tool.setuptools.packages.find]`
    resolves in *namespace* mode, where a folder at the root would enter the
    distribution even without `__init__.py`; ROSS keeps `docs` and `tools` out by
    restricting `include` to `ross*`. This reads the real configuration, so
    loosening it there is caught here."""
    base = _fake_repository(str(tmp_path))
    found = _discover(base, _ross_find_config())

    assert PACKAGE in found and PACKAGE + ".api" in found, (
        "the interface would not ship inside the ROSS wheel: %s" % found
    )
...
def test_the_folder_is_an_importable_package():
    """`ross.interface` is a package with a name, not a folder found by accident."""
    assert os.path.exists(os.path.join(ROOT, "__init__.py"))
    assert os.path.exists(os.path.join(ROOT, "tests", "__init__.py"))
...
def _extra(name):
    ...
    return _pyproject()["project"]["optional-dependencies"][name]
...
def _ross_requirements():
    ...
    if not os.path.exists(path):
        pytest.skip("not a repository checkout: ROSS's requirements.txt is absent")
...
def test_the_frontend_sweep_walks_the_whole_tree():
    ...
    from ross.interface.tests.frontend_source import FRONTEND, modules, relative
...
def test_the_workflow_wakes_for_our_folder_and_for_ross():
    ...
    """
    _, steps = _workflow()
    for pattern in ('"ross/**"', '"pyproject.toml"'):
...
def test_the_installed_workflow_is_the_one_we_keep():
    ...
    """
    installed = os.path.join(REPO, ".github", "workflows", "interface.yml")
