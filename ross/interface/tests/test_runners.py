...
from ross.interface.services.analysis import REGISTRY, get_runner
from ross.interface.services.analysis.ucs import HALF_A_RANGE
from ross.interface.tests.frontend_source import source as _frontend
...
def test_declared_plot_parameters_are_actually_used(name):
    ...
    source += without_comments(
        inspect.getsource(sys.modules["ross.interface.services.analysis.base"])
    )
...
def test_the_frequency_response_survives_an_empty_probe_table():
    ...
    js = inspect.getsource(
        sys.modules["ross.interface.services.analysis.freq_response"]
    )
...
def test_the_two_whirl_choices_are_refused_together_by_name():
    from ross.interface.services.analysis.modal import ONE_WHIRL_CHOICE
