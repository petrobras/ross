...
from ross.interface.services.worker.child import serve  # noqa: E402
...
def test_the_worker_is_started_as_this_same_program():
    ...
    from ross.interface.services.worker.host import worker_command
...
def test_the_measured_bytes_really_are_the_measured_numbers():
    ...
    """
    from ross.interface.services.worker.compare import decoded
...
def test_a_chart_equal_to_itself_is_identical_without_being_parsed():
    from ross.interface.services.worker.compare import IDENTICAL, compare
...
def test_campbell_last_bit_noise_is_close_and_not_a_difference():
    ...
    making.
    """
    from ross.interface.services.worker.compare import CLOSE, compare
...
def test_the_orbit_angle_that_moved_between_processes_is_a_difference():
    ...
    """
    from ross.interface.services.worker.compare import DIFFERENT, compare
...
def test_the_worst_difference_wins_even_when_it_comes_last():
    ...
    """
    from ross.interface.services.worker.compare import compare
...
def test_a_difference_just_above_the_threshold_is_still_a_difference():
    ...
    every test above.
    """
    from ross.interface.services.worker.compare import CLOSE, DIFFERENT, compare
...
def _resident(**kwargs):
    ...
    """
    from ross.interface.services.worker.resident import Resident
...
def test_a_message_goes_to_the_worker_when_there_is_one():
    from ross.interface.services.worker.resident import WORKER
...
def test_without_a_worker_the_same_function_answers_here():
    ...
    """
    from ross.interface.services.worker.resident import HERE
...
def test_a_worker_that_dies_costs_its_request_a_detour_and_not_a_failure():
    ...
    from ross.interface.services.worker.host import WorkerGone
    from ross.interface.services.worker.resident import HERE, WORKER
...
def test_the_conversation_ends_when_an_answer_belongs_to_another_question():
    ...
    question.
    """
    from ross.interface.services.worker.host import Worker, WorkerGone
...
def test_a_refusal_keeps_the_exception_class_it_had():
    ...
    """
    from ross.interface.services.worker.resident import WorkerFailure, raise_if_refused
...
def test_the_way_around_the_worker_is_the_function_the_worker_runs():
    ...
    assert "from ross.interface.services.worker.protocol import handle" in body, (
...
def test_the_route_hands_the_work_over_instead_of_doing_it():
    ...
    `from ross.interface.services.worker.resident import raise_if_refused` satisfied that, so
...
def test_a_worker_killed_on_purpose_does_not_send_the_work_back_here():
    ...
    """
    from ross.interface.services.worker.resident import Interrupted
...
def test_a_resident_given_a_worker_never_starts_one_of_its_own():
    ...
    from ross.interface.services.worker.host import WorkerGone
    from ross.interface.services.worker.resident import HERE, Resident
