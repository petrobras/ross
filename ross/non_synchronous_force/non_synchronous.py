...
def non_synchronous_force(F,m,a,k,omega,s,time_non_sync_params,n_frequencies_plot=None):
    ...
    if n_frequencies_plot:
        ...
        if type(n_frequencies_plot) == int:
            ...
            n_frequencies_plot = np.array(n_frequencies_plot).reshape((1,len(n_frequencies_plot)))
        ...
        for ii in range(1,len(n_frequencies_plot[0])):
            ...
            if len(index[ii]) != 0:
                ...
                aux_omega[0,idx_1:idx_1+len(idx_2[:,0])] = omega_ns[0,index[ii][idx_2[:,0]].T]
    ...
    for ii in range(len(omega_ns[0])):
        ...
        Amplitude_x.append((F*(mz*s**2*omega_ns[0,ii]**2 + ax*s*omega_ns[0,ii]**2 - kz))/(ax*az*s**2*omega_ns[0,ii]**4 - kx*kz + kx*mz*s**2*omega_ns[0,ii]**2 + kz*mx*s**2*omega_ns[0,ii]**2 - mx*mz*s**4*omega_ns[0,ii]**4))

        Amplitude_z.append((F*(mx*s**2*omega_ns[0,ii]**2 + az*s*omega_ns[0,ii]**2 - kx))/(ax*az*s**2*omega_ns[0,ii]**4 - kx*kz + kx*mz*s**2*omega_ns[0,ii]**2 + kz*mx*s**2*omega_ns[0,ii]**2 - mx*mz*s**4*omega_ns[0,ii]**4))
    ...
    if len(n_frequencies_plot) != 0:
        ...
        for ii in range(len(n_frequencies_plot[0])):
            ...
            title_string_amp_x = 'Amplitude on X axis for non synchronous force in ' + str(n_frequencies_plot[0,ii]) + ' rpm'

            title_string_amp_z = 'Amplitude on Z axis for non synchronous force in ' + str(n_frequencies_plot[0,ii]) + ' rpm'

            title_string_orbit = 'Orbits for non synchronous force in ' + str(n_frequencies_plot[0,ii]) + ' rpm'

            fig = make_subplots(rows=3, cols=1,subplot_titles=(title_string_amp_x, title_string_amp_z, title_string_orbit))

            fig.add_trace(go.Scatter(x=time_assync,y=np.array(qx[idx][:])), row=1, col=1)

            fig.add_trace(go.Scatter(x=time_assync,y=np.array(qz[idx][:])), row=2, col=1)
