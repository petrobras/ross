...
def test_run_freq_response(rotor1):
    ...
    phase = np.array(
        [
            ...
            [-4.907693e-03, -2.910273e-03],
            [-1.078713e-02, -6.285649e-03],
    ...
    assert_allclose(np.abs(results.freq_resp), magnitude, atol=1e-3)
    assert_allclose(np.angle(results.freq_resp), phase, atol=1e-2)
...
def test_run_unbalance_response(rotor1):
    ...
    phase = np.array(
        [
            [ 0.00000000e+00,  0.00000000e+00,  0.00000000e+00,  0.00000000e+00,
              0.00000000e+00,  0.00000000e+00,  0.00000000e+00,  0.00000000e+00],
            [-2.53354445e-02, -1.59613177e+00,  1.56908630e+00, -1.71003122e-03,  
            -1.30983310e-02, -1.58389466e+00,  1.57026803e+00, -5.28298177e-04], 
            [-5.66466799e-02, -1.62744301e+00,  1.56199595e+00, -8.80037629e-03, 
            -3.22287414e-02, -1.60302507e+00,  1.56451826e+00, -6.27806251e-03], 
            [-1.19748353e-01, -1.69054468e+00,  1.52434693e+00, -4.64493996e-02,  
            -8.32598757e-02, -1.65405620e+00,  1.52855816e+00, -4.22381653e-02], 
            [-2.68795148e+00,  2.02443750e+00, -1.01644832e+00, -2.58724464e+00,  
            -2.63955233e+00,  2.07283665e+00, -1.00994079e+00, -2.58073712e+00], 
            [-3.14113640e+00,  1.57125258e+00, -1.43940074e+00, -3.01019707e+00,  
            -3.08102983e+00,  1.63135915e+00, -1.42960239e+00, -3.00039872e+00], 
            [ 3.09481666e+00,  1.52402033e+00, -1.45248009e+00, -3.02327641e+00,  
            -3.11679365e+00,  1.59559533e+00, -1.43778318e+00, -3.00857950e+00], 
            [ 3.06699318e+00,  1.49619685e+00, -1.44079173e+00, -3.01158806e+00,  
            -3.13341498e+00,  1.57897401e+00, -1.41857535e+00, -2.98937168e+00]
        ]
    )
    # fmt: on
    ...
    assert_allclose(results.forced_resp[0, :8, :8].T, forced_resp, atol=1e-5)
    assert_allclose(np.abs(results.forced_resp[0, :8, :8]).T, magnitude, atol=1e-5)
    assert_allclose(np.angle(results.forced_resp[0, :8, :8]).T, phase, atol=1e-5)
