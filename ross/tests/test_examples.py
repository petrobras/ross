...
def test_example1_w_equals_0rpm():
    assert rotor_example1(0, 6).wn == pytest.approx(np.array([86.66, 86.66, 274.31, 274.31, 716.78, 716.78]), 1e-3)


def test_example1_w_equals_4000rpm():
    assert rotor_example1(4000*np.pi/30, 6).wn == pytest.approx(np.array([85.39, 87.80, 251.78, 294.71, 600.18, 827.08])
                                                                , 1e-3)
...
def rotor_example2(w, n_el):
    ...
    for i in range(n_el):
        shaft_elm.append(shaft.ShaftElement(L=1.5/n_el, material=steel, n=i, i_d=0, o_d=0.05))
    return rotor.Rotor(shaft_elm,
                       [disk.DiskElement.from_geometry(n=n_el, material=steel, width=0.07, i_d=0.05, o_d=0.35)],
                       [bse.BearingElement(n=0, kxx=10e6, kyy=10e6, cxx=0, cyy=0), bse.BearingElement(n=int((n_el/1.5)),
                                                                                                      kxx=10e6,
                                                                                                      kyy=10e6,
                                                                                                      cxx=0, cyy=0)],
                       w=w)
...
def test_example2_w_eq_0rpm():
    assert pytest.approx(rotor_example2(0, 6).wn, 1e-3) == np.array([90.14, 90.14, 630.73, 630.73, 830.43, 830.43])


def test_example2_w_eq_4000rpm():
    assert pytest.approx(rotor_example2(4000*np.pi / 30, 6).wn, 1e-3) == np.array([76.19, 103.91, 565.99,
...
def rotor_example3(w, n_el):
    ...
    for i in range(n_el):
        shaft_elm.append(shaft.ShaftElement(L=1.5/n_el, material=steel, n=i, i_d=0, o_d=0.05))
    disk0 = disk.DiskElement.from_geometry(n=(n_el/3), material=steel, width=0.07, i_d=0.05, o_d=0.28)
    disk1 = disk.DiskElement.from_geometry(n=(n_el/1.5), material=steel, width=0.07, i_d=0.05, o_d=0.35)
...
def test_example3_w_equals_0rpm():
    assert pytest.approx(rotor_example3(0, 48).wn, 1e-3) == np.array([82.65, 86.66, 254.52, 274.31, 679.49, 716.79])


def test_example3_w_equals_4000rpm():
    assert pytest.approx(rotor_example3(4000*np.pi/30, 48).wn, 1e-3) == np.array([82.33, 86.86, 239.64, 287.25, 583.49,
                                                                                 806.89])
...
def test_example4_w_equals_0rpm():
    wn_hertz = rotor_example4(0, 6).wn/(2*np.pi)
    wd_hertz = rotor_example4(0, 6).wd/(2*np.pi)
    assert pytest.approx(wn_hertz, 1e-2) == np.array([13.91, 13.91, 48.18, 48.18, 137.06, 137.06])
    assert pytest.approx(wd_hertz, 1e-2) == np.array([13.89, 13.89, 46.54, 46.54, 103.22, 103.22])


def test_example4_w_equals_4000rpm():
    wn_hertz = rotor_example4((4000*np.pi)/30, 6).wn/(2*np.pi)
    wd_hertz = rotor_example4((4000*np.pi)/30, 6).wd/(2*np.pi)
...
def test_example5_w_equals4000rpm():
    wn_hertz = rotor_example5((4000*np.pi)/30, 48).wn/(2*np.pi)
...
def test_example6_w_equals4000rpm():
    wn_hertz = rotor_example6((4000*np.pi)/30, 48).wn/(2*np.pi)
...
def rotor_example7(w, n_el):
    ...
    P.S.: Tapered shaft, damped bearings with anisotropic properties.
    ...
    shaft_elm = []
    for i in range(n_el):
        shaft_elm.append(shaft.ShaftElement(L=1.5/n_el, material=steel, n=i, i_d=0, o_d=0.025 + 0.015*(i/n_el)))

    disk0 = disk.DiskElement.from_geometry(n=(n_el/2), material=steel, width=0.07, i_d=0.05, o_d=0.28)
